...
public class HudOverlay implements HudRenderCallback {
    ...
    private static final KeyBinding toggleHudKey =
            KeyBindingHelper.registerKeyBinding(new KeyBinding("key.gameinfo.toggleHud", InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_F1, "category.gameinfo"));
    ...
    private void renderCoordinates(DrawContext drawContext, TextRenderer textRenderer, int xPos, int yPos, BlockPos pos, World world) {
        ...
        drawContext.drawTextWithShadow(textRenderer, "坐标: ", xPos, yPos, color);
        int width = textRenderer.getWidth("坐标: ");
    ...
    private void renderBiome(DrawContext drawContext, TextRenderer textRenderer, int xPos, int yPos, BlockPos pos, World world) {
        ...
        String biomeName = biomeId == null ? "未知生物群系" : I18n.translate("biome." + biomeId.getNamespace() + "." + biomeId.getPath());

        drawContext.drawTextWithShadow(textRenderer, "生物群系: ", xPos, yPos, color);
        int width = textRenderer.getWidth("生物群系: ");
        drawContext.drawTextWithShadow(textRenderer, biomeName, xPos + width, yPos, 0xFFFFFF);
    }
